...
- Make sure to use a commit-tag as described in semver
	- For small changes start the commit message with `fix:` to increment the minor tag in the version
   - For new features or larger changes start the commit message with `feat:` to incremenht the middle tag in the version
